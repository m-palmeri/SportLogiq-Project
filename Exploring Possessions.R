...
Hockey_Rink <- function(plot) {
  plot + geom_vline(xintercept = 25, color = 'blue', size = 2) +
   geom_vline(xintercept = 0, color = 'red', size = 2) +
   geom_vline(xintercept = -25, color = 'blue', size = 2) + xlim(-100, 100) + ylim(-42.5, 42.5)
}

# Determining Possession changes ----

#Seems that the only two delimiters of possessions are faceoffs, and loose puck recoveries (LPR)

Possession_id <- numeric(nrow(Events))
current_possession <- 0
#possession should be determined following a faceoff
faceoff <- TRUE
